...
$sql = "SELECT 
cart_products.id AS cart_id, cart_owner.user_id, account.id,account.Firstname,account.Lastname, account.Phone_nb, 
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<td>' . '<img src="../images/' . $row['category_name'] . '/' . $row['Picture'] . '" alt="Image" width="60px" height="60px">'  . '</td>' .
            ...
            . '<td>' . 
            '<a href="https://www.google.com/maps?q=' . $row["latitu"] . ',' . $row["longitu"] . '&z=15" target="_blank">Location</a>'.
