        ...
        <div class="to">
            ...
            <div class="categoriesContainer">
                <div class="container">
                    ...
                    $r_cat = mysqli_query($conn, $cat);
                    while ($category = mysqli_fetch_array($r_cat)) {
                        $res = "SELECT  i.Name as itemName,i.role  , c.Name as categoryName , i.* , c.* FROM items i,categories c
                         WHERE i.Cat_ID=c.Id and i.role = 1  and i.Cat_ID=" . $category['Id'] . " ORDER BY i.discount_percentage DESC, Add_Date DESC";
                        $r_res = mysqli_query($conn, $res);
                        if (mysqli_num_rows($r_res) > 0) {
                        ...
                        <div class="InnerContainer ">.';
                            while ($item = mysqli_fetch_array($r_res)) {
                                ...
                                if ($item['discount_percentage'] > 0 && $item['discount_end_date'] > $now) {
                                    ...
                                    echo  " <span style='color:Black;' >  &nbsp;</span><span style='color:green;'> $" . $item['Price_after_discount'] 
                                    . " &nbsp; </span>  <span style='color:Black;' >  &nbsp;</span> <span style='color:gray;text-decoration: line-through;'>$" . $item['Price'] . "</span>";
